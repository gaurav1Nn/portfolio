...
const Contact: React.FC = () => {
  ...
  // Add status state for form submission
  ...
  // Updated handleSubmit with FormSubmit integration
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/57681dbd4674530011c1e210bd6c6c4c", {
        method: "POST",
        headers: {
          ...
          <div className="contact-content">
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-sm space-y-6">
              ...
              <button
                ...
                className={`submit-btn w-full text-white px-6 py-4 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 flex items-center justify-center space-x-2 ${
                  status.submitting 
              ...
              {status.error && (
                <div className="mt-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg animate-fade-in">
                  ...
                  <p className="text-sm mt-1">Please try again or email me directly at nilawargaurav@gmail.com</p>
                </div>
              )}
            </form>
          </div>
